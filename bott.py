...
@dp.callback_query_handler(text_contains="film")
async def process_video_command(call: CallbackQuery):
    await call.answer(cache_time=60)
    callback_data = call.data
    logging.info(f"callback_data='{callback_data}'")
    await bot.delete_message(call.from_user.id, call.message.message_id)
    await call.message.answer('🤔 что же выбрать', reply_markup=keyboardmainmenu.filmClava)
...
async def nextSERIA(message:types.Message): 
    ...
    if db.statesearch(message.from_user.id) == 1:
        await bot.send_message(message.from_user.id, text=f"серия:2")
        ...
        search=db.statesearch(message.from_user.id)
        try:
            await bot.send_message(message.from_user.id, text=f"серия: {search}")
